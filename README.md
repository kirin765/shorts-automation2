...
`subtitle_position`으로 자막 위치를 조정할 수 있습니다.
- 형식: `left|center|right,top|middle|bottom` (예: `center,middle`)
- 기본값: `center,middle`
- 위치별 정렬은 ffmpeg libass `Alignment`로 매핑됩니다.

- 폰트 탐색 우선순위
  - `font_file`이 유효하면 해당 파일/폰트를 최우선 사용
  - macOS: `AppleSDGothicNeo`, `Apple Gothic`, `NanumGothic`, `Noto Sans CJK`
  - WSL/Windows 폰트(`C:/Windows/Fonts` 계열)
  - 마지막으로 `fc-match`의 ko 폰트 후보(`Noto Sans CJK KR`, `Noto Sans KR`, `Apple SD Gothic Neo`) 
  - `font_file`이 비어 있거나 실패하면 시스템 기본 폰트로 폴백

- 한글이 네모(□)로 보일 때 체크리스트
  - `font_file` 경로를 삭제하고 기본 탐색 로그 확인
  - 로그의 `fontfile=`/`font=` 값이 실제 폰트 경로인지 확인
  - 제목과 자막에서 모두 같은 계열 폰트가 쓰이는지 확인
...
동기화 체크 가드
- SRT 마지막 타임스탬프와 오디오 길이 차이가 `subtitle_sync_drift_tolerance`(기본 `0.5`)를 넘으면 보정/재생성/폴백합니다.
- `subtitle_sync_repair` 제어 키(선택): `subtitle_sync_repair`, `subtitle_sync_repair_max_drift`, `subtitle_sync_max_scale_delta`
...
- `assets/fonts/NotoSansKR-Bold.ttf` : 폰트(선택, 없으면 자동 탐색 사용)
- `output/*.mp4` : 결과물
...
- `subtitle_margin_v`, `subtitle_font_size`, `subtitle_outline`, `subtitle_align`, `subtitle_vshift`로 자막 가독성 조정
- `video_preset`, `video_crf`, `video_bitrate`, `audio_bitrate`로 렌더 품질 조정

## subtitle 정렬 프리셋(빠른 적용)
`notes/subtitle_center_presets.json`에 `center` 기준 템플릿을 넣어두었습니다.

예: 중간 정렬 + 1회용 고정값으로 실행

```bash
cp config.example.json /tmp/config_run.json
cat > /tmp/update_short_cfg.py <<'PY'
import json
from pathlib import Path
cfg = json.loads(Path('/tmp/config_run.json').read_text())
for k,v in {
  ...
  cfg[k]=v
Path('/tmp/config_run.json').write_text(json.dumps(cfg,indent=2,ensure_ascii=False))
PY
python3 /tmp/update_short_cfg.py
NO_UPLOAD=1 python3 run_short.py --config /tmp/config_run.json --job jobs/today.json
```

직접 프리셋 사용 시 `notes/subtitle_center_presets.json`의 JSON을 참고해 `subtitle_*` 값만 원하는 값으로 반영해서 쓰면 됩니다.
